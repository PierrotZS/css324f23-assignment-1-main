...
def h3(s):
    ...
    for idx in range(n):
        if board[idx] != 0:  # Skip the blank tile
            ...
            # Calculate the target row and column of the tile
            target_row, target_col = divmod(board[idx] - 1, 3)

            # Calculate the absolute differences between the current and target row/column
            row_diff = abs(current_row - target_row)
            col_diff = abs(current_col - target_col)

            # Add the differences to the heuristic value
